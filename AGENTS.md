...
- **Usage data layer exists** (`data/usage/`): per-app daily limits + today's used/earned time, with atomic accrual and free daily reset. Nothing writes to it yet — the app picker (limits) and foreground monitor (usage) come next.
...
- **Tests:** unit tests cover the usage limit math and repository rollover (`UsageMathTest`, `UsageRepositoryTest` — the latter uses an in-memory fake `UsageDao` + injected clock). Still untested despite being written for it: `classify`, `sensitivityToMinConfidence`, `toGallerySections`.
- **Not yet built:** an app picker, foreground-app detection, the real limit-reached block trigger, the capture→earn-time reward loop, and live Home/Stats.
