...
- **The reward loop is still open**: a cat capture doesn't yet credit time or dismiss the block — that's next.
...
- **Not yet built:** the capture→earn-time reward loop (`CameraEvent.Saved` should credit bonus minutes to the blocked app and dismiss the block — `CameraRoute` still drains events without acting), and live Home/Stats.
